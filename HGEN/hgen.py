...
# Number of classes/views for the added datasets
CLASS_LOOKUP = {
    'football': 6,
    'enron_mid': 10,
    'nell2_sampled': 4
...
parser.add_argument("--dataset", choices=["syn_100", "syn_200", "syn_500", "PubMed", "IMDB_movie", "DBLP_four_area"] + list(CLASS_LOOKUP.keys()), 
...
def data_preprocess(val_share, test_share, seed, directory=args.dataset):
    ...
    train_ones, val_ones, val_zeros, test_ones, test_zeros = utils.train_val_test_split_adjacency(
            _A_obs, val_share, test_share, seed, undirected=False, connected=False, asserts=False)
...
def filter_path(path, max_val):
    return [x for x in path if all(y < max_val for y in x)]
class HINDataset(torch.utils.data.Dataset):
    def __init__(self, node_attribute, num_nodes, max_path_len, dataset, lcc):
        ...
        elif self.dataset in CLASS_LOOKUP:
            self.num_classes = CLASS_LOOKUP[self.dataset] + 1
        else:
    ...
    def cache(self, node_attribute, num_nodes):
        # print(Counter(node_attribute))
        print('num nodes:', num_nodes)
        path_len_2 = filter_path(pickle.load(open(self.dataset+"/path_len_2.p", "rb")), num_nodes)
        path_len_3 = filter_path(pickle.load(open(self.dataset+"/path_len_3.p", "rb")), num_nodes)
        if Path(self.dataset+"/path_len_4.p").exists():
            path_len_4 = filter_path(pickle.load(open(self.dataset+"/path_len_4.p", "rb")), num_nodes)
        else:
            print('Paths of len 4 not found. Assuming that we get OOM if we try to compute this.')
            path_len_4 = []
        
        paths = np.array(path_len_2 + path_len_3 + path_len_4)
        g = np.random.Generator(np.random.PCG64())
        sampled_paths = paths[g.choice(len(paths), min(24000, len(paths)), replace=False)]
        real_walk_data = []
        for path in sampled_paths:
            ...
            # print(f'temp_type: {temp_type.shape[1]}')
...
class Generator(nn.Module):
    def __init__(self, node_type, N, node_embs, device, args):
        ...
        if args.dataset in ["syn_100", "syn_200", "syn_500"]:
            self.node_classes = 4
        elif args.dataset in CLASS_LOOKUP:
            self.node_classes = CLASS_LOOKUP[args.dataset] + 1
        else:
            self.node_classes = 5
        ...
        self.types = [
            torch.tensor(node_type[x], dtype=torch.float).to(self.device) for x in range(len(node_type))
        ]

        self.lin_nodes = nn.ModuleList([
            nn.Linear(self.hidden_units, self.node_emb_size) for _ in range(self.node_classes)
        ])
    ...
    def forward(self, z):
        ...
        for i in range(self.max_path_len):
            ...
            for j, x in enumerate(torch.argmax(output_type, dim=1)):
                if x == self.node_classes - 1:
                    # print(f'[hit!] x: {x}')
                    temp_node.append(torch.tensor(self.N).to(self.device))
                elif x < self.node_classes - 1:
                    # print(f'x: {x}')
                    temp_output_node = self.lin_nodes[x](out.squeeze(0)[j])
                    ...
                    temp_node.append(self.types[x][candidate])
                else:
                    print(f'\n\n\n[miss!] x: {x}\n\n\n')
                # if x==0:
            ...
            # print(f'temp_node: {temp_node.shape}')
        ...
        # print(f'outputs_type: {outputs_type.shape}')
        outputs_node = torch.stack(outputs_node, dim=1)
        # print(f'outputs_node: {outputs_node.shape}')
...
class Discriminator(nn.Module):
    def __init__(self, N, device, args):
        ...
        self.max_path_len = args.max_path_len
        if args.dataset in ["IMDB", "syn_100", "syn_200", "syn_500"]:
            self.node_classes = 4
        elif args.dataset in CLASS_LOOKUP:
            self.node_classes = CLASS_LOOKUP[args.dataset] + 1
        else:
    ...
    def forward(self, z):
        ...
        # print(f'Testing: {self.max_path_len} and {self.node_classes} vs {input_type.size()} -> {input_type.view(self.batch_size, -1).size()}')
...
def generate_walks():
    ...
    for i in range(n_eval_iters):
        ...
        # print('synthetic_type: ', synthetic_type)
        # print('synthetic_node: ', synthetic_node)

        removal_target = 4
        if args.dataset in CLASS_LOOKUP:
            removal_target = CLASS_LOOKUP[args.dataset]
        smpls_type += utils.delete_from_tail(synthetic_type, removal_target)
...
def meta_path_frequency(smpls_type_2, smpls_type_3, smpls_type_4):
    ...
    len_2, len_3, len_4 = len_2[:len(len_2)//2], len_3[:len(len_3)//2], len_4[:len(len_4)//2]
    
    return dict(len_2 + len_3 + len_4)


def evaluation(orig_A, test_A, syn_A):
    #orig_G = nx.from_scipy_sparse_matrix(orig_A)
...
real_walks = HINDataset(_X_obs, _N, args.max_path_len, args.dataset, lcc)
...
def train():
    ...
    for epoch in range(args.n_epochs):
        for i, (real_node_type, real_node_seq) in enumerate(dataloader):
            ...
            # print(f'(real_node_type, real_node_seq): ({real_node_type.shape}, {real_node_seq.shape})')
...
def square_graph(graph):
    min_dim = min(graph.shape)
    return graph[:min_dim, :min_dim]

if __name__ == '__main__':
    print('Start Training...')
    cache_path = f'runs/{args.dataset}_epochs-{args.n_epochs}.pkl'

    if Path(cache_path).exists():
        print('Found cached data...')
        smpls_type, smpls_node = load_pickle(cache_path)
        print('Loaded pickle!')
    else:
        train()
        if args.save:
        ...
        save_pickle((smpls_type, smpls_node), cache_path)
        # print(smpls_type)
    #smpls_type, smpls_node = pickle.load(open("results/smpls_type_dblp.p","rb")), pickle.load(open("results/smpls_node_dblp.p","rb"))
    smpls_type_2 = [i for i in smpls_type if i.shape[0] == 2 and CLASS_LOOKUP[args.dataset] not in i]
    smpls_type_3 = [i for i in smpls_type if i.shape[0] == 3 and CLASS_LOOKUP[args.dataset] not in i]
    smpls_type_4 = [i for i in smpls_type if i.shape[0] == 4 and CLASS_LOOKUP[args.dataset] not in i]
    ...
    test_graph = square_graph(test_graph)
    syn_graph = utils.heterogeneous_graph_assemble(score_matrix, test_graph.sum(), meta_path_freq, node_type)
    print(_A_obs.shape, syn_graph.shape, test_graph.shape)
    save_pickle(syn_graph, f'sample_syn_graph_{args.dataset}.pkl')
